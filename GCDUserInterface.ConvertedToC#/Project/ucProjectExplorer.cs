...
using GCDUserInterface.SurveyLibrary;
using GCDCore.UserInterface.Project;
using GCDCore.UserInterface.Project.ProjectTreeNode;
	...
	public partial class ucProjectExplorer
	{
		...
		private void LoadTree(string sSelectedNodeTag = "", SortSurveyBy eSortSurveyBy = SortSurveyBy.SurveyDateDsc)
		{
			...
			if (ProjectManager.Project is GCDProject) {
				LoadTree(ref treProject, ProjectManager.Project, false, sSelectedNodeTag, eSortSurveyBy);
			}

		}


		public static void LoadTree(ref TreeView tre, GCDProject rProject, bool bCheckboxes, string sSelectedNodeTag = "", SortSurveyBy eSortSurveyBy = SortSurveyBy.SurveyDateDsc)
		{
			try {
				TreeNode nodProject = tre.Nodes.Add(GCDNodeTypes.Project.ToString, rProject.Name, GCDNodeTypes.Project, GCDNodeTypes.Project);
				...
				TreeNode nodInputs = nodProject.Nodes.Add(GCDNodeTypes.InputsGroup.ToString, m_sGroupInputs, GCDNodeTypes.InputsGroup);
				...
				TreeNode nodSurveysGroup = nodInputs.Nodes.Add(GCDNodeTypes.SurveysGroup.ToString, "DEM Surveys", GCDNodeTypes.SurveysGroup);
				...
				switch (eSortSurveyBy) {
					case SortSurveyBy.NameAsc:
						orderedSurveys = rProject.DEMsSortByName(true);
						break;
					case SortSurveyBy.NameDsc:
						orderedSurveys = rProject.DEMsSortByName(false);
						break;
					case SortSurveyBy.SurveyDateAsc:
						orderedSurveys = rProject.DEMsSortByDate(true);
						break;
				...
				foreach (DEMSurvey dem in orderedSurveys) {
					TreeNode nodSurvey = nodSurveysGroup.Nodes.Add(GCDNodeTypes.DEMSurvey + "_" + dem.Name, dem.Name, GCDNodeTypes.DEMSurvey);
					...
					// Associated surfaces
					TreeNode nodAssocGroup = nodSurvey.Nodes.Add(GCDNodeTypes.AssociatedSurfaceGroup.ToString, m_sAssocSurfaces, GCDNodeTypes.AssociatedSurfaceGroup);
					...
					foreach (AssocSurface assoc in dem.AssocSurfaces.Values) {
						TreeNode nodAssoc = nodAssocGroup.Nodes.Add(GCDNodeTypes.AssociatedSurface.ToString + "_" + assoc.Name, assoc.Name, GCDNodeTypes.AssociatedSurface);
					...
					// Error surfaces
					TreeNode nodErrorGroup = nodSurvey.Nodes.Add(GCDNodeTypes.ErrorSurfaceGroup.ToString, m_sErrorSurfaces, GCDNodeTypes.ErrorSurfaceGroup);
					...
					foreach (ErrorSurface errSurf in dem.ErrorSurfaces.Values) {
						TreeNode nodError = nodErrorGroup.Nodes.Add(GCDNodeTypes.ErrorSurface.ToString + "_" + errSurf.Name, errSurf.Name, GCDNodeTypes.ErrorSurface);
				...
				TreeNode AnalNode = nodProject.Nodes.Add(GCDNodeTypes.AnalysesGroup.ToString, "Analyses", GCDNodeTypes.AnalysesGroup);
				...
				TreeNode CDNode = AnalNode.Nodes.Add(GCDNodeTypes.ChangeDetectionGroup.ToString, "Change Detection", GCDNodeTypes.ChangeDetectionGroup);
				...
				foreach (DoDBase rDoD in rProject.DoDs.Values) {
					...
					TreeNode nodDoD = theParent.Nodes.Add(rDoD.Name, rDoD.Name, GCDNodeTypes.DoD);
					...
					foreach (BudgetSegregation rBS in rDoD.BudgetSegregations.Values) {
						nodDoD.Expand();
					...
					foreach (string sPolygonMask in sMaskDict.Keys) {
						...
						foreach (BudgetSegregation rBS in rDoD.BudgetSegregations.Values) {
							...
							if (string.Compare(sPolygonMask, rBS.PolygonMask.FullName, true) == 0) {
								...
								// Budget Segregation
								TreeNode nodBS = nodMask.Nodes.Add(GCDNodeTypes.BudgetSegregation.ToString + "_" + rBS.Name.ToString, rBS.Name, GCDNodeTypes.BudgetSegregation);
		...
		private GCDNodeTypes GetNodeType(TreeNode aNode)
		{
			...
			switch (sType) {
				case GCDNodeTypes.Project.ToString:
					eType = GCDNodeTypes.Project;
					break;
				case GCDNodeTypes.InputsGroup.ToString:
					eType = GCDNodeTypes.InputsGroup;
					break;
				case GCDNodeTypes.SurveysGroup.ToString:
					eType = GCDNodeTypes.SurveysGroup;
					break;
				case GCDNodeTypes.DEMSurvey.ToString:
					eType = GCDNodeTypes.DEMSurvey;
					break;
				case GCDNodeTypes.AssociatedSurfaceGroup.ToString:
					eType = GCDNodeTypes.AssociatedSurfaceGroup;
					break;
				case GCDNodeTypes.AssociatedSurface.ToString:
					eType = GCDNodeTypes.AssociatedSurface;
					break;
				case GCDNodeTypes.ErrorSurfaceGroup.ToString:
					eType = GCDNodeTypes.ErrorSurfaceGroup;
					break;
				case GCDNodeTypes.ErrorSurface.ToString:
					eType = GCDNodeTypes.ErrorSurface;
					break;
				case GCDNodeTypes.AnalysesGroup.ToString:
					eType = GCDNodeTypes.AnalysesGroup;
					break;
				case GCDNodeTypes.ChangeDetectionGroup.ToString:
					eType = GCDNodeTypes.ChangeDetectionGroup;
					break;
				case GCDNodeTypes.ChangeDetectionDEMPair.ToString:
					eType = GCDNodeTypes.ChangeDetectionDEMPair;
					break;
				case GCDNodeTypes.DoD.ToString:
					eType = GCDNodeTypes.DoD;
					break;
				//Case GCDNodeTypes.SummaryXML.ToString : eType = GCDNodeTypes.SummaryXML
				case GCDNodeTypes.BudgetSegregationGroup.ToString:
					eType = GCDNodeTypes.BudgetSegregationGroup;
					break;
				case GCDNodeTypes.BudgetSegregationMask.ToString:
					eType = GCDNodeTypes.BudgetSegregationMask;
					break;
				case GCDNodeTypes.BudgetSegregation.ToString:
					eType = GCDNodeTypes.BudgetSegregation;
					break;
				case GCDNodeTypes.ReservoirGroup.ToString:
					eType = GCDNodeTypes.ReservoirGroup;
					break;
				case GCDNodeTypes.Reservoir.ToString:
					eType = GCDNodeTypes.Reservoir;
					break;
				case GCDNodeTypes.AOIGroup.ToString:
					eType = GCDNodeTypes.AOIGroup;
					break;
				case GCDNodeTypes.AOI.ToString:
				...
				if (nodSelected is TreeNode) {
					...
					switch (eType) {
						...
						case GCDNodeTypes.AssociatedSurfaceGroup:
							...
							break;
						case GCDNodeTypes.AssociatedSurface:
							DEMSurvey DEM = (DEMSurvey)((ProjectTreeNode)nodSelected.Parent.Parent.Tag).Item;
							frm = new frmAssocSurfaceProperties(DEM, null);
		...
		private void AddBudgetSegregationToolStripMenuItem1_Click(System.Object sender, System.EventArgs e)
		{
			try {
				...
				if (nodSelected is TreeNode) {
					...
					UI.BudgetSegregation.frmBudgetSegProperties frm = new UI.BudgetSegregation.frmBudgetSegProperties((DoDBase)treDod.Item);
					if (frm.ShowDialog() == DialogResult.OK) {
						LoadTree();
						UI.BudgetSegregation.frmBudgetSegResults frmResults = new UI.BudgetSegregation.frmBudgetSegResults(frm.BudgetSeg);
						frmResults.ShowDialog();
					}
				}
			} catch (Exception ex) {
