	...
	public partial class frmImportRaster
	{
		...
		private bool ValidateForm()
		{
			...
			if (string.IsNullOrEmpty(txtName.Text)) {
				Interaction.MsgBox("The raster name cannot be empty.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				return false;
			}

			if (!ucRaster.SelectedItem is GCDConsoleLib.Raster) {
				Interaction.MsgBox("You must select a raster to import. Use the browse button if the raster you want is not already in the map and dropdown list.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				return false;
			}
			...
			if (string.IsNullOrEmpty(txtRasterPath.Text)) {
				if (m_ePurpose == ImportRasterPurposes.StandaloneTool) {
					Interaction.MsgBox("The output raster path cannot be empty. Click the Save button to specify an output raster path.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				} else {
					Interaction.MsgBox("The " + ucRaster.Noun + " path cannot be empty. Try using a different name.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				}
				return false;
			} else {
				if (System.IO.File.Exists(txtRasterPath.Text)) {
					Interaction.MsgBox("The project raster path already exists. Try using a different name for the raster.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					return false;
				} else {
					string sExtension = System.IO.Path.GetExtension(txtRasterPath.Text);
					if (string.Compare(sExtension, ".tif", true) != 0) {
						Interaction.MsgBox("This tool can only currently produce GeoTIFF rasters. Please provide an output raster path ending with '.tif'", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
						return false;
					}
				}
			}
			...
			if (valCellSize.Value <= 0) {
				Interaction.MsgBox("The cell size must be greater than or equal to zero.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				return false;
			}

			if ((valRight.Value - valLeft.Value) < valCellSize.Value) {
				Interaction.MsgBox("The right edge of the extent must be at least one cell width more than the left edge of the extent.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				return false;
			}

			if (valTop.Value - valBottom.Value < valCellSize.Value) {
				Interaction.MsgBox("The top edge of the extent must be at least one cell width more than the bottom edge of the extent.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				return false;
			}

			ResamplingMethods eType = ((naru.db.NamedObject)cboMethod.SelectedItem()).ID;
			if (RequiresResampling()) {
				if (eType != ResamplingMethods.Bilinear) {
					if (eType == ResamplingMethods.None) {
						Interaction.MsgBox("The input raster requires resampling. Please select the desired resampling method.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					} else {
						Interaction.MsgBox("Only bilinear interpolation is currently functional within the GCD.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					}
					return false;
				}
			} else {
				if (cboMethod.SelectedIndex != m_nNoInterpolationIndex) {
					Interaction.MsgBox("The raster is orthogonal and divisible with the specified output. No interpolation is required. Select \"None\" in the interpolation method drop down.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					return false;
				}
			}
			...
			if (ucRaster.SelectedItem is GCDConsoleLib.Raster) {
				...
				if (GetSafeOriginalRaster(ref gOriginalRaster)) {
					if (valPrecision.Enabled) {
						...
						//
						int nPrecision = 1;
						double fCellSize = gOriginalRaster.Extent.CellWidth;
						for (i = 0; i <= 10; i++) {
							double fTest = fCellSize * Math.Pow(10, i);
					...
					if (ucRaster.SelectedItem is GCDConsoleLib.Raster) {
						...
						switch (m_ePurpose) {
							...
							case ImportRasterPurposes.ErrorCalculation:
								...
								break;
							default:
								Interaction.MsgBox("Unhandled import raster purpose: " + m_ePurpose.ToString(), MsgBoxStyle.Exclamation, GCDCore.Properties.Resources.ApplicationNameLong);
								break;
						}
					}
				}
				...
				if (System.IO.File.Exists(txtRasterPath.Text)) {
					...
					if (GetSafeOriginalRaster(ref gRaster)) {
						...
						if (theDir.Exists) {
							...
							double cellHeight = valCellSize.Value;
		...
		private void UpdateOutputRowsColsHeightWidth()
		{
			...
			double fProjHeight = (valTop.Value - valBottom.Value);
			double fProjWidth = (valRight.Value - valLeft.Value);
		...
		private void UpdateOriginalRasterExtentFormatting()
		{
			...
			if (m_OriginalExtent is GCDConsoleLib.ExtentRectangle && valCellSize.Value > 0) {
				double fValue = Math.IEEERemainder(m_OriginalExtent.Left, valCellSize.Value);
		...
		private void valPrecision_ValueChanged(object sender, System.EventArgs e)
		{
			...
			valCellSize.Increment = Math.Pow(10, (decimal.Negate(valPrecision.Value)));
			valCellSize.Minimum = Math.Pow(10, decimal.Negate(valPrecision.Value));
