...
namespace GCDUserInterface.SurveyLibrary
{
	...
	public partial class frmAssocSurfaceProperties
	{
		...
		private void SurfacePropertiesForm_Load(System.Object sender, System.EventArgs e)
		{
			...
			ttpTooltip.SetToolTip(btnHelp, GCDUserInterface.My.Resources.ttpHelp);
		...
		private AssociatedSurfaceMethods GetAssociatedSurfaceType(nSurfaceID)
		{
		...
		private bool ValidateForm()
		{
			...
			if (string.IsNullOrEmpty(txtName.Text)) {
				Interaction.MsgBox("Please provide a name for the associated surface.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
				txtName.Focus();
				return false;
			} else {
				if (!DEM.IsAssocNameUnique(txtName.Text, m_Assoc)) {
					Interaction.MsgBox("The name '" + txtName.Text + "' is already in use by another associated surface within this survey. Please choose a unique name.", MsgBoxStyle.Exclamation, GCDCore.Properties.Resources.ApplicationNameLong);
					txtName.Select();
					return false;
				}
			...
			if (string.IsNullOrEmpty(txtProjectRaster.Text)) {
				if (m_eMethod == AssociatedSurfaceMethods.Browse) {
					Interaction.MsgBox("You must either browse and select an existing raster for this associated surface, or choose to generate a slope or point density raster from the DEM Survey raster.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					return false;
				}
			} else {
				if (m_Assoc == null && System.IO.File.Exists(txtProjectRaster.Text)) {
					Interaction.MsgBox("The associated surface project raster path already exists. Changing the name of the associated surface will change the raster path.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					return false;
				}
			}

			if (cboType.SelectedItem == null) {
				Interaction.MsgBox("Please select an error type to continue.", MsgBoxStyle.Exclamation, GCDCore.Properties.Resources.ApplicationNameLong);
				cboType.Focus();
				return false;
			}
		...
		{
			if (!m_ImportForm is frmImportRaster) {
		...
		private void SlopeTypeSelected(AssociatedSurfaceMethods eType)
		{
			...
			for (int i = 0; i <= cboType.Items.Count - 1; i++) {
				if (((naru.db.NamedObject)cboType.Items[i]).ID == m_eMethod) {
					cboType.SelectedIndex = i;
