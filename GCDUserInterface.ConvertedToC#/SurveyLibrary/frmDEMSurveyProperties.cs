	...
	public partial class frmDEMSurveyProperties
	{
		...
		private void SurveyPropertiesForm_Load(System.Object sender, System.EventArgs e)
		{
			...
			if (DEM.IsSingleSurveyMethod) {
				...
				if (DEM.MethodMask is IO.FileInfo && DEM.MethodMask.Exists) {
		...
		private bool ValidateForm()
		{
			...
			if (rdoSingle.Checked) {
				if (string.IsNullOrEmpty(cboSingle.Text)) {
					Interaction.MsgBox("You must select a survey method type.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					cboSingle.Select();
					return false;
				}
			} else {
				if (string.IsNullOrEmpty(txtMask.Text)) {
					Interaction.MsgBox("You must select a survey mask feature class.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					btnBrowseMask.Select();
					return false;
				}

				if (string.IsNullOrEmpty(cboIdentify.Text)) {
					Interaction.MsgBox("You must select a survey method identifier field.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong);
					cboIdentify.Select();
					return false;
				}
			}
		...
		private void txtMask_TextChanged(object sender, System.EventArgs e)
		{
			...
			cboIdentify.Items.AddRange(gMask.Fields.Values.Where((GCDConsoleLib.VectorField s) => s.Type.Equals(GCDConsoleLib.GDalFieldType.StringField)));
		...
		private void LoadRasterProperties()
		{
			...
			sRasterProperties += Constants.vbNewLine + "Left: " + _with1.Left.ToString("#,##0.#");
			sRasterProperties += Constants.vbNewLine + "Top: " + _with1.Top.ToString("#,##0.#");
			sRasterProperties += Constants.vbNewLine + "Right: " + _with1.Right.ToString("#,##0.#");
			sRasterProperties += Constants.vbNewLine + "Bottom: " + _with1.Bottom.ToString("#,##0.#");
			sRasterProperties += Constants.vbNewLine;
			sRasterProperties += Constants.vbNewLine + "Cell size: " + _with1.CellWidth;
			sRasterProperties += Constants.vbNewLine;
			sRasterProperties += Constants.vbNewLine + "Width: " + (_with1.Right - _with1.Left).ToString("#,##0.#");
			sRasterProperties += Constants.vbNewLine + "Height: " + (_with1.Top - _with1.Bottom).ToString("#,##0.#");
			sRasterProperties += Constants.vbNewLine;
			sRasterProperties += Constants.vbNewLine + "Rows: " + _with1.rows.ToString("#,##0");
			sRasterProperties += Constants.vbNewLine + "Columns: " + _with1.cols.ToString("#,##0");

			sRasterProperties += Constants.vbNewLine + Constants.vbNewLine;
			...
			try {
				// File Size
				sRasterProperties += Constants.vbNewLine + "Raster file size: " + naru.os.File.GetFormattedFileSize(DEM.Raster.RasterPath);
			} catch (Exception ex) {
				// Do nothing. 
			}
		...
		private void ViewAssociatedSurface(System.Object sender, EventArgs e)
		{
			...
			AssocSurface assoc = (AssocSurface)grdAssocSurface.SelectedRows().Item(0).DataBoundItem;
		...
		private void btnDeleteAssociatedSurface_Click(System.Object sender, System.EventArgs e)
		{
			...
			if (MessageBox.Show("Are you sure you want to remove the selected associated surface from the GCD Project?" + " This will also delete the raster associated with this surface.", "Deleted associated surface?", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes) {
				DEM.DeleteAssociatedSurface((AssocSurface)grdAssocSurface.SelectedRows().Item(0).DataBoundItem);
			}
		...
		private void btnDeleteErrorSurface_Click(System.Object sender, System.EventArgs e)
		{
			...
			if (MessageBox.Show("Are you sure you want to delete the selected error surface from the GCD project?" + " This will also delete the raster associated with this error surface.", "Delete Error Surface", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes) {
				DEM.DeleteErrorSurface((ErrorSurface)grdErrorSurfaces.SelectedRows().Item(0).DataBoundItem);
