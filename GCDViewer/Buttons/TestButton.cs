    ...
    internal class TestButton : Button
    {
        ...
        protected override async void OnClick()
        {
            ...
            await QueuedTask.Run(async () =>
            {
                ...
                var colorRamps = style.SearchColorRamps("Partial Spectrum");
                
                var cimColorRamp = colorRamps[0].ColorRamp;
                foreach(var ramp in colorRamps)
                {
                    if (string.Compare(ramp.Name, "Partial Spectrum", true) == 0)
                    {
                        cimColorRamp = ramp.ColorRamp;
                        break;
                    }
                }
                ...
                //stretchColorizer.StretchClasses[1] = new CIMRasterStretchClass() { Value = max / 2 };
