...
Namespace GISCode.GCD
    ...
    Public Class GCDArcMapManager
        ...
        Public Sub New(ByVal pArcMap As ESRI.ArcGIS.Framework.IApplication, Optional ByVal fDefaultDEMTransparency As Double = 40, Optional ByVal pMapDocument As IMapDocument = Nothing)
            m_pArcMap = pArcMap
            m_fDefaultDEMTransparency = fDefaultDEMTransparency
            m_pMapDocument = pMapDocument
        End Sub

        Private ReadOnly Property MapDocument As IMapDocument
            Get
                Dim pMapDocument As IMapDocument
                If TypeOf m_pMapDocument Is IMapDocument Then
                    pMapDocument = m_pMapDocument
                Else
                    pMapDocument = m_pArcMap.Document
                End If
                Return pMapDocument
            End Get
        ...
        Public Function AddDEM(demRow As ProjectDS.DEMSurveyRow) As IRasterLayer
            ...
            If GISDataStructures.Raster.Exists(sHillShadePath) Then
            ...
            Dim sSymbology As String = GISDataStructures.RasterGCD.GetSymbologyLayerFile(ArcMap.RasterLayerTypes.DEM)
        ...
        Public Function AddAssociatedSurface(assocRow As ProjectDS.AssociatedSurfaceRow) As IRasterLayer
            ...
            Dim gAssociatedRaster As GISDataStructures.Raster = New GISDataStructures.Raster(sRasterPath)
            ...
            ElseIf eType = ArcMap.RasterLayerTypes.Roughness Then
                ...
                'Dim sSymbology As String = GISDataStructures.RasterGCD.GetSymbologyLayerFile(eType)
        ...
        Public Function AddErrSurface(errRow As ProjectDS.ErrorSurfaceRow) As IRasterLayer
            ...
            Dim gErrRaster As GISDataStructures.Raster = New GISDataStructures.Raster(sRasterPath)
        ...
        Public Function AddDoD(dodRow As ProjectDS.DoDsRow, Optional bThresholded As Boolean = True) As IRasterLayer
            ...
            Dim sSymbology As String = GISDataStructures.RasterGCD.GetSymbologyLayerFile(ArcMap.RasterLayerTypes.DoD)
            Dim dTransparency As Double = -1
            If My.Settings.TransparencyAnalysesLayers Then
                dTransparency = My.Settings.AutoTransparencyValue
            End If

            Dim gDoDRaster As GISDataStructures.Raster = New GISDataStructures.Raster(sRasterPath)
        ...
        Public Function AddDoDTemp(theProjectRow As ProjectDS.ProjectRow, gRaster As GISDataStructures.RasterDirect, sName As String) As IRasterLayer
        ...
        Public Function AddPropErr(dodRow As ProjectDS.DoDsRow) As IRasterLayer
            ...
            Dim gPropErrRaster As GISDataStructures.Raster = New GISDataStructures.Raster(sRasterPath)
        ...
        Public Function AddProbabilityRaster(dodRow As ProjectDS.DoDsRow, sProbabilityRasterPath As String) As IRasterLayer
            ...
            Dim gProbabilityRaster As GISDataStructures.Raster = New GISDataStructures.Raster(sRasterPath)
        ...
        Private Function AddToMapRaster(sSource As String,
            ...
            If Not GISDataStructures.Raster.Exists(sSource) Then
                Return Nothing
            End If
            '
            '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        Private Function AddToMapVector(sSource As String, sDisplayName As String, pGrpLyr As IGroupLayer, Optional fTransparency As Double = -1) As ILayer
            ...
            Dim pWS As ESRI.ArcGIS.Geodatabase.IFeatureWorkspace = pWSF.OpenFromFile(GISDataStructures.VectorDataSource.GetWorkspacePath(sSource), 0)
        ...
        '                Dim gTempRaster As GISDataStructures.Raster = New GISDataStructures.Raster(sTempRasterPath)
