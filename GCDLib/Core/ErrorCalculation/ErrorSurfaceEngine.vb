...
Namespace GISCode.GCD
    ...
    Public Class ErrorSurfaceEngine
        ...
        Private ReadOnly Property DEMRaster As GISDataStructures.RasterDirect
            Get
                Dim gDEMRaster As New GISDataStructures.RasterDirect(GCD.GCDProject.ProjectManager.GetAbsolutePath(m_ErrorSurfaceRow.DEMSurveyRow.Source))
                Return gDEMRaster
            End Get
        End Property
        ...
        Public Function CreateErrorSurfaceRaster() As GISDataStructures.RasterDirect

            ' Create the name for the final error surface raster
            Dim sErrorSurfaceRasterPath As String = GCD.GCDProject.ProjectManager.OutputManager.ErrorSurfaceRasterPath(m_ErrorSurfaceRow.DEMSurveyRow.Name, m_ErrorSurfaceRow.Name, True)
            Dim gErrorSurface As GISDataStructures.RasterDirect = Nothing
                ...
                Case AssociatedsurfaceErrorType
                    ...
                    For Each rAssoc As ProjectDS.AssociatedSurfaceRow In m_ErrorSurfaceRow.DEMSurveyRow.GetAssociatedSurfaceRows
                        If rAssoc.AssociatedSurfaceID = m_ErrorSurfaceRow.GetMultiErrorPropertiesRows.First.AssociatedSurfaceID Then
                            Dim sAssocPath As String = GCD.GCDProject.ProjectManager.GetAbsolutePath(rAssoc.Source)
            ...
            gErrorSurface = New GISDataStructures.RasterDirect(sErrorSurfaceRasterPath)
            m_ErrorSurfaceRow.Source = GCD.GCDProject.ProjectManager.GetRelativePath(sErrorSurfaceRasterPath)
            GCD.GCDProject.ProjectManager.save()

            ' Build raster pyramids
            If GISCode.GCD.RasterPyramidManager.AutomaticallyBuildPyramids(GISCode.GCD.RasterPyramidManager.PyramidRasterTypes.ErrorSurfaces) Then
                GISCode.GCD.RasterPyramidManager.PerformRasterPyramids(GISCode.GCD.RasterPyramidManager.PyramidRasterTypes.ErrorSurfaces, sErrorSurfaceRasterPath)
            End If
        ...
        Private Sub CreateMultiMethodErrorSurface(sOutputRasterPath As String)
            ...
            For Each aMethod As ProjectDS.MultiErrorPropertiesRow In m_ErrorSurfaceRow.GetMultiErrorPropertiesRows
                Dim sMethodRaster As String = GCD.GCDProject.ProjectManager.OutputManager.ErrorSurfarceMethodRasterPath(m_ErrorSurfaceRow.DEMSurveyRow.Name, m_ErrorSurfaceRow.Name, aMethod.Method, True)

                Dim sMethodMask As String = GCD.GCDProject.ProjectManager.OutputManager.ErrorSurfarceMethodRasterMaskPath(m_ErrorSurfaceRow.DEMSurveyRow.Name, m_ErrorSurfaceRow.Name, aMethod.Method, True)
                Dim gMaskRaster As GISDataStructures.RasterDirect = CreateRasterMask(aMethod.Method, sMethodMask)
            ...
            If Not String.IsNullOrEmpty(sAllMethodRasters) Then
                Dim gDEM As New GISDataStructures.RasterDirect(GCD.GCDProject.ProjectManager.GetAbsolutePath(m_ErrorSurfaceRow.DEMSurveyRow.Source))
                ...
                Dim eResult As RasterManager.RasterManagerOutputCodes = RasterManager.Mosaic(sAllMethodRasters, sMosaicWithoutMask, GCD.GCDProject.ProjectManager.GCDNARCError.ErrorString)
                If eResult <> RasterManagerOutputCodes.PROCESS_OK Then
                    ...
                    ex.Data("Error Message") = GCD.GCDProject.ProjectManager.GCDNARCError.ErrorString.ToString
                    Throw ex
                End If

                Try
                    eResult = RasterManager.Mask(sMosaicWithoutMask, gDEM.FullPath, sOutputRasterPath, GCD.GCDProject.ProjectManager.GCDNARCError.ErrorString)
                    If eResult <> RasterManagerOutputCodes.PROCESS_OK Then
                        Dim exInner As New Exception(GCDProject.ProjectManager.GCDNARCError.ErrorString.ToString)
                ...
                m_ErrorSurfaceRow.Source = GCD.GCDProject.ProjectManager.GetRelativePath(sOutputRasterPath)
                m_ErrorSurfaceRow.Type = MultipleErrorType
            End If

        End Sub

        Private Function CreateUniformErrorSurface(fErrorValue As Double, gRasterMask As GISDataStructures.RasterDirect, sOutputRasterPath As String) As GISDataStructures.RasterDirect

            Dim gErrorSurface As GISDataStructures.RasterDirect = Nothing

            Try
                ' Do the conditional geoprocessing.
                Dim eResult As GCDCore.GCDCoreOutputCodes = GCDCore.UniformError(gRasterMask.FullPath, sOutputRasterPath, fErrorValue, GCDProject.ProjectManager.GCDNARCError.ErrorString)
                ...
                gErrorSurface = New GISDataStructures.RasterDirect(sOutputRasterPath)
        ...
        '''</remarks>
        Private Function CreateAssociatedErrorSurface(sAssociatedSurfacePath As String, gRasterMask As GISDataStructures.RasterDirect, sOutputRasterPath As String) As GISDataStructures.RasterDirect

            Dim gErrorSurface As GISDataStructures.RasterDirect = Nothing

            Try
                Dim eResult As RasterManager.RasterManagerOutputCodes = RasterManager.Mask(sAssociatedSurfacePath, gRasterMask.FullPath, sOutputRasterPath, GCDProject.ProjectManager.GCDNARCError.ErrorString)
                If eResult <> GCDCoreOutputCodes.PROCESS_OK Then
        ...
        Private Sub CreateFISErrorSurface(sFISRuleDefinitionFileName As String, gReferenceRaster As GISDataStructures.RasterDirect, sOutputRasterPath As String, bClipToMask As Boolean)
            ...
            For Each aFISRuleFile As FISLibrary.FISTableRow In GCD.GCDProject.ProjectManager.fisds.FISTable.Rows
            ...
            For Each FISInput As ProjectDS.FISInputsRow In m_ErrorSurfaceRow.GetFISInputsRows
                ...
                If String.Compare(FISInput.FIS, sFISFuleFilePath, True) = 0 OrElse String.Compare(FISInput.FIS, IO.Path.GetFileNameWithoutExtension(sFISFuleFilePath)) = 0 Then
                    ...
                    Dim sSQL As String = GCD.GCDProject.ProjectManager.ds.AssociatedSurface.DEMSurveyIDColumn.ColumnName & "=" & m_ErrorSurfaceRow.DEMSurveyRow.DEMSurveyID
                    sSQL &= " AND " & GCD.GCDProject.ProjectManager.ds.AssociatedSurface.NameColumn.ColumnName & "='" & FISInput.AssociatedSurface & "'"

                    Dim AssociatedSurface As ProjectDS.AssociatedSurfaceRow = GCD.GCDProject.ProjectManager.ds.AssociatedSurface.Select(sSQL).First

                    sInputs &= FISInput.FISInput & ";" & GCD.GCDProject.ProjectManager.GetAbsolutePath(AssociatedSurface.Source) & ";"
                End If
            Next
            ...
            Try
                ...
                eResult = GCDCore.CreateFISError(DEMRaster.FullPath, sFISFuleFilePath, sInputs, sFullFISRaster, _
                                                    GCDProject.ProjectManager.OutputManager.OutputDriver, _
                                                    GCDProject.ProjectManager.OutputManager.NoData, GCD.GCDProject.ProjectManager.GCDNARCError.ErrorString)
        ...
        Private Function CreateRasterMask(ByVal sMethodName As String, ByVal sOutputRasterMaskPath As String) As GISDataStructures.RasterDirect
            ...
            Dim gMaskFeatures As GISDataStructures.PolygonDataSource = CopyFeatures(GCD.GCDProject.ProjectManager.GetAbsolutePath(m_ErrorSurfaceRow.DEMSurveyRow.MethodMask), OutShapefile, WhereClause)
            ...
            Try
                ...
                gResult = New GISDataStructures.RasterDirect(sOutputRasterMaskPath)
            Catch ex As Exception
        ...
        ''' <remarks></remarks>
        Private Function CopyFeatures(sMaskFeatureClass As String, sOutputFeatureClass As String, Optional sWhereClause As String = "") As GISDataStructures.PolygonDataSource

            Dim gPolygonMask As New GISDataStructures.PolygonDataSource(sMaskFeatureClass)
            Dim gOutput As GISDataStructures.PolygonDataSource = GISDataStructures.VectorDataSource.CreateFeatureClass(sOutputFeatureClass, GISDataStructures.BasicGISTypes.Polygon, False, gPolygonMask.SpatialReference)
            ...
            Dim gResult As New GISDataStructures.PolygonDataSource(sOutputFeatureClass)
            Return gResult
