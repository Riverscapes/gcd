...
Public Class frmErrorCalculation2
    ...
    Private m_dErrorCalculationProperties As Dictionary(Of String, ErrorCalculation.ErrorCalcPropertiesBase)
    ...
    Private Sub frmErrorCalculation2_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        For Each rFIS As FISLibrary.FISTableRow In GCD.GCDProject.ProjectManagerUI.fisds.FISTable
            cboFIS.Items.Add(New ListItem(rFIS.FISID, rFIS.Name))
        Next
        ...
        For Each aSurface As ProjectDS.AssociatedSurfaceRow In m_rDEMSurvey.GetAssociatedSurfaceRows
            colCombo.Items.Add(New ListItem(aSurface.AssociatedSurfaceID, aSurface.Name))

            ' Also load any error surfaces into the associated error surface combo box.
            'If String.Compare(aSurface.Type, "Error Surface", True) = 0 Then
            cboAssociated.Items.Add(New ListItem(aSurface.AssociatedSurfaceID, aSurface.Name))
            'End If
        Next
        ...
        Try
            ...
            Dim sDEMPath As String = GCD.GCDProject.ProjectManagerUI.GetAbsolutePath(m_rDEMSurvey.Source)
            Dim gDEM As New GISDataStructures.Raster(sDEMPath)
            Dim l As New LinearUnitClass(GISCode.NumberFormatting.GetLinearUnitsFromESRI(gDEM.LinearUnits))
            rdoUniform.Text = rdoUniform.Text & " " & l.GetUnitsAsString(True)
        Catch ex As Exception
    ...
    ''' <remarks></remarks>
    Public Sub New(pArcMap As IApplication, rDEMSurvey As ProjectDS.DEMSurveyRow)
    ...
    ''' <remarks></remarks>
    Public Sub New(pArcMap As IApplication, rErrorSurface As ProjectDS.ErrorSurfaceRow)
    ...
    Private Sub LoadSurveyMethods()
        ...
        m_dErrorCalculationProperties = New Dictionary(Of String, ErrorCalculation.ErrorCalcPropertiesBase)
        ...
        If TypeOf m_rErrorSurface Is ProjectDS.ErrorSurfaceRow Then
            ...
            For Each rProperties As ProjectDS.MultiErrorPropertiesRow In m_rErrorSurface.GetMultiErrorPropertiesRows
                Select Case rProperties.ErrorType
                    ...
                    Case GISCode.GCD.ErrorSurfaceEngine.UniformErrorString
                        m_dErrorCalculationProperties.Add(rProperties.Method, New ErrorCalculation.ErrorCalcPropertiesUniform(rProperties.Method, rProperties._Error))

                    Case GISCode.GCD.ErrorSurfaceEngine.AssociatedsurfaceErrorType
                        m_dErrorCalculationProperties.Add(rProperties.Method, New ErrorCalculation.ErrorCalcPropertiesAssoc(rProperties.Method, rProperties.AssociatedSurfaceID))
                    ...
                    Case Else
                        ...
                        For Each rFISRuleFile As FISLibrary.FISTableRow In GCD.GCDProject.ProjectManager.fisds.FISTable.Rows
                        ...
                        m_dErrorCalculationProperties.Add(rProperties.Method, New ErrorCalculation.ErrorCalcPropertiesFIS(rProperties.Method, nFISID, sFISFuleFilePath, dFISInputs))
        ...
        Else
            ...
            If IsSingleMethodError Then
                ...
                m_dErrorCalculationProperties(sSurveyType) = New ErrorCalculation.ErrorCalcPropertiesUniform(sSurveyType, fErrorValue)
            Else
                If m_rDEMSurvey.IsMethodMaskFieldNull Then
                    Throw New Exception("Multi method DEM with no method mask field defined.")
                Else
                    Dim sMethodMask As String = GCD.GCDProject.ProjectManager.GetAbsolutePath(m_rDEMSurvey.MethodMask)
                    Dim gMethodMask As New GISDataStructures.PolygonDataSource(sMethodMask)
    ...
    Private Sub LoadErrorCalculationMethods()
        ...
        For Each errProps As ErrorCalculation.ErrorCalcPropertiesBase In m_dErrorCalculationProperties.Values
    ...
    Private Function GetDefaultErrorValue(sMethod As String) As Double
        ...
        Dim dRows As SurveyTypes.SurveyTypesRow() = ProjectManager.surveyds._SurveyTypes.Select("Name = '" & sMethod & "'")
    ...
    Private Sub UpdateGridWithErrorProperties(nNewRow)
        ...
        If Not String.IsNullOrEmpty(sMethod) Then
            If m_dErrorCalculationProperties.ContainsKey(sMethod) Then
                ...
                Dim eErrorProperties As ErrorCalculation.ErrorCalcPropertiesBase = m_dErrorCalculationProperties(sMethod)
                If TypeOf eErrorProperties Is ErrorCalculation.ErrorCalcPropertiesUniform Then
                    rdoUniform.Checked = True
                    valUniform.Value = DirectCast(eErrorProperties, ErrorCalculation.ErrorCalcPropertiesUniform).UniformErrorValue

                    cboFIS.SelectedIndex = -1
                    cboAssociated.SelectedIndex = -1

                ElseIf TypeOf eErrorProperties Is ErrorCalculation.ErrorCalcPropertiesAssoc Then
                    rdoAssociated.Checked = True
                    cboFIS.SelectedIndex = -1

                    For i As Integer = 0 To cboAssociated.Items.Count - 1
                        If DirectCast(cboAssociated.Items(i), ListItem).ID = DirectCast(eErrorProperties, ErrorCalculation.ErrorCalcPropertiesAssoc).AssociatedSurfaceID Then
                            cboAssociated.SelectedIndex = i
                            Exit For
                        End If
                    Next
                Else
                    Dim eFISProperties As ErrorCalculation.ErrorCalcPropertiesFIS = eErrorProperties
                    rdoFIS.Checked = True

                    'cboFIS.SelectedValue = eFISProperties.FISID
                    For i As Integer = 0 To cboFIS.Items.Count - 1
                        If String.Compare(DirectCast(cboFIS.Items(i), ListItem).Name, eFISProperties.FISRuleName, True) = 0 Then
    ...
    Private Sub UpdateFISGrid()
        ...
        If rdoFIS.Checked Then
            If TypeOf cboFIS.SelectedItem Is ListItem Then
                Dim l As ListItem = cboFIS.SelectedItem
                For Each rFIS As FISLibrary.FISTableRow In GCD.GCDProject.ProjectManagerUI.fisds.FISTable
                    If l.ID = rFIS.FISID Then
                        ' This is the FIS selected in the combo box
                        Dim theFISRuleFile As New GCD.ErrorCalculation.FISRuleFile(rFIS.Path)
                        ...
                        For Each sInput As String In theFISRuleFile.FISInputs
                            ...
                            If lErr.Count = 1 Then
                                Dim errProps As ErrorCalculation.ErrorCalcPropertiesBase = m_dErrorCalculationProperties(lErr(0).Cells(0).Value)

                                ' Only proceed if the error surface definition is a FIS
                                If TypeOf errProps Is ErrorCalculation.ErrorCalcPropertiesFIS Then
                                    Dim fisProps As ErrorCalculation.ErrorCalcPropertiesFIS = errProps
    ...
    Private Function SaveErrorProperties() As Boolean
        ...
        If r.Count = 1 Then
            ...
            If rdoUniform.Checked Then
                ...
                m_dErrorCalculationProperties(sSurveyMethod) = New ErrorCalculation.ErrorCalcPropertiesUniform(sSurveyMethod, valUniform.Value)
            ElseIf rdoAssociated.Checked Then

                If TypeOf cboAssociated.SelectedItem Is ListItem Then
                    ' Create a new associated surface error properties
                    m_dErrorCalculationProperties(sSurveyMethod) = New ErrorCalculation.ErrorCalcPropertiesAssoc(sSurveyMethod, DirectCast(cboAssociated.SelectedItem, ListItem).ID)
                Else
                ...
                Else
                    ...
                    For Each rFIS As FISLibrary.FISTableRow In GCD.GCDProject.ProjectManagerUI.fisds.FISTable
                        If String.Compare(rFIS.Name, cboFIS.Text, True) = 0 Then
                            m_dErrorCalculationProperties(sSurveyMethod) = New ErrorCalculation.ErrorCalcPropertiesFIS(sSurveyMethod, rFIS.FISID, rFIS.Path, dInputs)
                            Exit For
                        End If
                    Next
                End If
    ...
    Private Sub txtName_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtName.TextChanged
        ...
        Else
            ...
            If Not String.IsNullOrEmpty(txtName.Text) Then
                sRasterPath = GCD.GCDProject.ProjectManager.OutputManager.ErrorSurfaceRasterPath(m_rDEMSurvey.Name, txtName.Text, False)
                sRasterPath = GCD.GCDProject.ProjectManagerUI.GetRelativePath(sRasterPath)
            End If
    ...
    Private ReadOnly Property ErrorSurfaceType As String
        Get
            Dim sType As String = GCD.ErrorSurfaceEngine.UniformErrorString

            Dim nFISCount As Integer = 0
            Dim nUniCount As Integer = 0
            Dim nAssoc As Integer = 0

            For Each errProp As ErrorCalculation.ErrorCalcPropertiesBase In m_dErrorCalculationProperties.Values
                If TypeOf errProp Is ErrorCalculation.ErrorCalcPropertiesUniform Then
                    nUniCount += 1
                ElseIf TypeOf errProp Is ErrorCalculation.ErrorCalcPropertiesAssoc Then
                    nAssoc += 1
                Else
                    nFISCount += 1
                End If
            ...
            If nUniCount = 1 AndAlso nFISCount = 0 AndAlso nAssoc = 0 Then
                sType = GCD.ErrorSurfaceEngine.UniformErrorString
            ElseIf nFISCount = 1 AndAlso nUniCount = 0 AndAlso nAssoc = 0 Then
                sType = GCD.ErrorSurfaceEngine.FISErrorType
            ElseIf nAssoc = 1 AndAlso nUniCount = 0 AndAlso nFISCount = 0 Then
                sType = GCD.ErrorSurfaceEngine.AssociatedsurfaceErrorType
            Else
                sType = GCD.ErrorSurfaceEngine.MultipleErrorType
            End If
    ...
    Private Sub btnOK_Click(sender As Object, e As System.EventArgs) Handles btnOK.Click
        ...
        Else
            ...
            m_rErrorSurface = GCD.GCDProject.ProjectManagerUI.ds.ErrorSurface.NewErrorSurfaceRow
            m_rErrorSurface.Name = txtName.Text
            m_rErrorSurface.DEMSurveyRow = m_rDEMSurvey
            m_rErrorSurface.Source = txtRasterPath.Text
            m_rErrorSurface.Type = ErrorSurfaceType
            GCD.GCDProject.ProjectManagerUI.ds.ErrorSurface.AddErrorSurfaceRow(m_rErrorSurface)
            ...
            For Each sSurveyMethod As String In m_dErrorCalculationProperties.Keys
                ...
                If TypeOf m_dErrorCalculationProperties(sSurveyMethod) Is ErrorCalculation.ErrorCalcPropertiesUniform Then
                ...
                If TypeOf m_dErrorCalculationProperties(sSurveyMethod) Is ErrorCalculation.ErrorCalcPropertiesAssoc Then
                    nAssociatedSurfaceID = DirectCast(m_dErrorCalculationProperties(sSurveyMethod), ErrorCalculation.ErrorCalcPropertiesAssoc).AssociatedSurfaceID
                End If
                ...
                If TypeOf m_dErrorCalculationProperties(sSurveyMethod) Is ErrorCalculation.ErrorCalcPropertiesFIS Then
                    sErrorType = DirectCast(m_dErrorCalculationProperties(sSurveyMethod), ErrorCalculation.ErrorCalcPropertiesFIS).FISRuleName
                End If

                GCD.GCDProject.ProjectManagerUI.ds.MultiErrorProperties.AddMultiErrorPropertiesRow(sSurveyMethod, fError, m_rErrorSurface, sErrorType, nAssociatedSurfaceID) ' m_dErrorCalculationProperties(sSurveyMethod).ErrorType)

                ' Now add all the FIS inputs to the FIS table
                If TypeOf m_dErrorCalculationProperties(sSurveyMethod) Is ErrorCalculation.ErrorCalcPropertiesFIS Then
                    Dim errProps As ErrorCalculation.ErrorCalcPropertiesFIS = m_dErrorCalculationProperties(sSurveyMethod)
                    For Each sInput As String In errProps.FISInputs.Keys
                        ...
                        For Each rAssoc As ProjectDS.AssociatedSurfaceRow In m_rDEMSurvey.GetAssociatedSurfaceRows
                            If rAssoc.AssociatedSurfaceID = errProps.FISInputs(sInput) Then
                                GCD.GCDProject.ProjectManagerUI.ds.FISInputs.AddFISInputsRow(m_rErrorSurface, sInput, rAssoc.Name, errProps.FISRuleName)
                                Exit For
                            End If
                        Next
                    Next
            ...
            Try
                Dim errEngine As New GISCode.GCD.ErrorSurfaceEngine(m_rErrorSurface)
                errEngine.CreateErrorSurfaceRaster()

                If My.Settings.AddOutputLayersToMap Then
                    GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddErrSurface(m_rErrorSurface)
                End If
            Catch ex As Exception
                Me.DialogResult = Windows.Forms.DialogResult.None
                Windows.Forms.Cursor.Current = Cursors.Default
                GCD.GCDProject.ProjectManagerUI.ds.ErrorSurface.RemoveErrorSurfaceRow(m_rErrorSurface)
        ...
        Try
            GCD.GCDProject.ProjectManagerUI.save()
        Catch ex As Exception
            Me.DialogResult = Windows.Forms.DialogResult.None
            Windows.Forms.Cursor.Current = Cursors.Default
    ...
    Private Function GetFISInputString(ByRef errProps As ErrorCalculation.ErrorCalcPropertiesFIS) As String
        ...
        For Each sFISInput As String In errProps.FISInputs.Keys
            For Each rAssoc As ProjectDS.AssociatedSurfaceRow In m_rDEMSurvey.GetAssociatedSurfaceRows
                If rAssoc.AssociatedSurfaceID = errProps.FISInputs(sFISInput) Then
                    sInputs &= sFISInput & ";" & GCD.GCDProject.ProjectManager.GetAbsolutePath(rAssoc.Source) & ";"
                    Exit For
                End If
            Next
        Next
