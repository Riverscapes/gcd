...
Public Class frmSurveyDateTime
    ...
    Public Sub New()
        ...
        InitializeComponent()
        m_dateTime = New SurveyDateTime
    End Sub

    Public Property SurveyDateTime As SurveyDateTime
        Get
            Return m_dateTime
        End Get
        Set(value As SurveyDateTime)
            m_dateTime = value
        End Set
    End Property
    ...
    Private Sub frmSurveyDateTime_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        cboYear.Items.Add(New ListItem(0, "YYYY"))
        cboYear.SelectedIndex = 0
        For nYear As Integer = 1970 To Now.Year + 5
            nIndex = cboYear.Items.Add(New ListItem(nYear, nYear))
            If nYear = m_dateTime.Year Then
                cboYear.SelectedIndex = nIndex
            End If
        Next

        cboMonth.Items.Add(New ListItem(0, "MM"))
        cboMonth.SelectedIndex = 0
        For nMonth As Integer = 1 To 12
            Dim dt As New DateTime(1970, nMonth, 1)
            nIndex = cboMonth.Items.Add(New ListItem(nMonth, dt.ToString("MMM")))
        ...
        cboHour.Items.Add(New ListItem(-1, "HH"))
        cboHour.SelectedIndex = 0
        For nHour As Integer = 0 To 24
            nIndex = cboHour.Items.Add(New ListItem(nHour, nHour.ToString("00")))
            If nHour = m_dateTime.Hour Then
                cboHour.SelectedIndex = nIndex
            End If
        Next

        cboMinute.Items.Add(New ListItem(-1, "MM"))
        cboMinute.SelectedIndex = 0
        For nMin As Integer = 0 To 59
            nIndex = cboMinute.Items.Add(New ListItem(nMin, nMin.ToString("00")))
    ...
    Private Function ValidateForm() As Boolean
        ...
        If DirectCast(cboYear.SelectedItem, ListItem).ID = 0 AndAlso DirectCast(cboMonth.SelectedItem, ListItem).ID <> 0 Then
        ...
        If DirectCast(cboYear.SelectedItem, ListItem).ID = 0 AndAlso DirectCast(cboMonth.SelectedItem, ListItem).ID = 0 AndAlso DirectCast(cboDay.SelectedItem, ListItem).ID <> 0 Then
        ...
        If DirectCast(cboHour.SelectedItem, ListItem).ID = -1 AndAlso DirectCast(cboMinute.SelectedItem, ListItem).ID > -1 Then
    ...
    Private Sub cmdSave_Click(sender As Object, e As System.EventArgs) Handles cmdSave.Click
        ...
        m_dateTime = New SurveyDateTime
        m_dateTime.Year = DirectCast(cboYear.SelectedItem, ListItem).ID
        m_dateTime.Month = DirectCast(cboMonth.SelectedItem, ListItem).ID
        m_dateTime.Day = DirectCast(cboDay.SelectedItem, ListItem).ID

        m_dateTime.Hour = DirectCast(cboHour.SelectedItem, ListItem).ID
        m_dateTime.Minute = DirectCast(cboMinute.SelectedItem, ListItem).ID
    ...
    Private Sub cboYear_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles _
        cboYear.SelectedIndexChanged, _
        cboMonth.SelectedIndexChanged

        Dim nCurrentDay As Integer = 0
        If TypeOf cboDay.SelectedItem Is ListItem Then
            nCurrentDay = DirectCast(cboDay.SelectedItem, ListItem).ID
        End If
    ...
    Private Sub ReLoadDaysOfMonth(nSelectDay As Integer)
        ...
        If TypeOf cboYear.SelectedItem Is ListItem Then
            Dim nYear As UShort = DirectCast(cboYear.SelectedItem, ListItem).ID
            If nYear > 0 Then
                If TypeOf cboMonth.SelectedItem Is ListItem Then
                    Dim nMonth As Byte = DirectCast(cboMonth.SelectedItem, ListItem).ID
                    If nMonth > 0 Then
        ...
        cboDay.Items.Clear()
        cboDay.Items.Add(New ListItem(0, "DD"))
        cboDay.SelectedIndex = 0
        For nDay As Integer = 1 To nMaxDays
            Dim nIndex As Integer = cboDay.Items.Add(New ListItem(nDay, nDay))
