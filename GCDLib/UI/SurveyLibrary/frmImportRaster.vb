...
Public Class frmImportRaster
    ...
    Public ReadOnly Property OriginalExtent As GISDataStructures.ExtentRectangle
        Get
    ...
    Public ReadOnly Property StringFormat As String
        Get
            ...
            If TypeOf valPrecision Is Windows.Forms.NumericUpDown Then
    ...
    Public Sub New(pArcMap As ESRI.ArcGIS.Framework.IApplication, gReferenceRaster As GISDataStructures.RasterDirect, ByRef demSurveyRow As ProjectDS.DEMSurveyRow, ePurpose As ImportRasterPurposes, sNoun As String)
        ...
        ucRaster.Initialize(pArcMap, sNoun)
    End Sub
    ...
    Private Sub ImportRasterForm_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        cboMethod.Items.Add(New ListItem(ResamplingMethods.Bilinear, "Bilinear Interpolation"))
        cboMethod.Items.Add(New ListItem(ResamplingMethods.Cubic, "Cubic Convolution"))
        cboMethod.Items.Add(New ListItem(ResamplingMethods.NaturalNeighbours, "Natural Neighbours"))
        cboMethod.Items.Add(New ListItem(ResamplingMethods.NearestNeighbour, "Nearest Neighbour"))
        m_nNoInterpolationIndex = cboMethod.Items.Add(New ListItem(ResamplingMethods.None, "None (straight cell-wise copy)"))
        ...
        If GCDExtension.GetGCDExtension(ucRaster.ArcMap).IsCurrentProject Then
            valPrecision.Value = GCD.GCDProject.ProjectManager.CurrentProject.Precision.ToString
        End If
        ...
        Else
            ...
            If m_ePurpose = ImportRasterPurposes.DEMSurvey Then
                If TypeOf m_gReferenceRaster Is GISDataStructures.RasterDirect Then
    ...
    Private Function ValidateForm() As Boolean
        ...
        If String.IsNullOrEmpty(txtName.Text) Then
            MsgBox("The raster name cannot be empty.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
            Return False
        End If
        ...
        If TypeOf ucRaster.SelectedItem Is GISDataStructures.Raster Then
            ...
            End If
        Else
            MsgBox("You must select a raster to import. Use the browse button if the raster you want is not already in the map and dropdown list.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
            Return False
        End If

        If String.IsNullOrEmpty(txtRasterPath.Text) Then
            If m_ePurpose = ImportRasterPurposes.StandaloneTool Then
                MsgBox("The output raster path cannot be empty. Click the Save button to specify an output raster path.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
            Else
                MsgBox("The " & ucRaster.Noun & " path cannot be empty. Try using a different name.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
            End If
            Return False
        Else
            ...
            Else
                ...
                If String.Compare(sExtension, ".tif", True) <> 0 Then
                    MsgBox("This tool can only currently produce GeoTIFF rasters. Please provide an output raster path ending with '.tif'", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
                    Return False
                End If
            End If
        End If
        ...
        If valCellSize.Value <= 0 Then
            MsgBox("The cell size must be greater than or equal to zero.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
            Return False
        End If

        If (valRight.Value - valLeft.Value) < valCellSize.Value Then
            MsgBox("The right edge of the extent must be at least one cell width more than the left edge of the extent.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
            Return False
        End If

        If valTop.Value - valBottom.Value < valCellSize.Value Then
            MsgBox("The top edge of the extent must be at least one cell width more than the bottom edge of the extent.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
            Return False
        End If

        If TypeOf cboMethod.SelectedItem Is ListItem Then
            Dim lItem As ListItem = DirectCast(cboMethod.SelectedItem(), ListItem)
            ...
            If RequiresResampling() Then
                If eType <> ResamplingMethods.Bilinear Then
                    If eType = ResamplingMethods.None Then
                        MsgBox("The input raster requires resampling. Please select the desired resampling method.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
                    Else
                        MsgBox("Only bilinear interpolation is currently functional within the GCD.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
                    End If
                    Return False
                End If
            Else
                If cboMethod.SelectedIndex <> m_nNoInterpolationIndex Then
                    MsgBox("The raster is orthogonal and divisible with the specified output. No interpolation is required. Select ""None"" in the interpolation method drop down.", MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
                    Return False
                End If
            End If
        End If
        ...
        If (m_ePurpose = ImportRasterPurposes.DEMSurvey AndAlso valPrecision.Enabled = False) Or (m_ePurpose = ImportRasterPurposes.AssociatedSurface) Or (m_ePurpose = ImportRasterPurposes.ErrorCalculation) Then
            ...
            If Not GCD.GCDProject.ProjectManager.CurrentProject.DisplayUnits Is Nothing Then
                If TypeOf ucRaster.SelectedItem Is GISDataStructures.Raster Then
                    Dim pLinearUnits As LinearUnitClass = New LinearUnitClass(GISCode.NumberFormatting.GetLinearUnitsFromESRI(ucRaster.SelectedItem.LinearUnits))
                    If Not String.Compare(pLinearUnits.ToString, GCD.GCDProject.ProjectManager.CurrentProject.DisplayUnits) = 0 Then
                        MsgBox(String.Format("The linear units of the selected raster {0} does not match the linear units {1} of the GCD Project." & vbCrLf & vbCrLf & "Please select a raster that has the same linear units as the GCD Project.",
                                             pLinearUnits.ToString, GCD.GCDProject.ProjectManager.CurrentProject.DisplayUnits), MsgBoxStyle.Information, My.Resources.ApplicationNameLong)
                        Return False
                    End If
                End If
            End If
    ...
    Private Sub OriginalRasterChanged()
        ...
        '
        If TypeOf ucRaster.SelectedItem Is GISDataStructures.Raster Then
            Dim gOriginalRaster As GISDataStructures.RasterDirect = Nothing
            If GetSafeOriginalRaster(gOriginalRaster) Then
                ...
                If valPrecision.Enabled Then
                    ...
                    Dim fCellSize As Double = gOriginalRaster.CellSize
                    For i = 0 To 10
                ...
                m_OriginalExtent = New GISDataStructures.ExtentRectangle(gOriginalRaster.Extent.Top, gOriginalRaster.Extent.Left, gOriginalRaster.Extent.Right, gOriginalRaster.Extent.Bottom)
                ...
                txtOrigRows.Text = gOriginalRaster.Rows.ToString("#,##0")
                txtOrigCols.Text = gOriginalRaster.Columns.ToString("#,##0")
                ...
                If Not (TypeOf m_gReferenceRaster Is GISDataStructures.RasterDirect AndAlso m_ePurpose <> ImportRasterPurposes.DEMSurvey) Then
                ...
                If String.IsNullOrEmpty(txtName.Text) Then
                    txtName.Text = ucRaster.SelectedItem.DatasetName
                Else
                    UpdateRasterPath()
                End If
            End If
        ...
        If TypeOf m_gReferenceRaster Is GISDataStructures.RasterDirect AndAlso (m_ePurpose <> ImportRasterPurposes.DEMSurvey OrElse m_ePurpose = ImportRasterPurposes.StandaloneTool) Then
        ...
        ElseIf TypeOf m_gReferenceRaster Is GISDataStructures.RasterDirect AndAlso (m_ePurpose = ImportRasterPurposes.DEMSurvey OrElse m_ePurpose = ImportRasterPurposes.StandaloneTool) Then
            UpdateOutputExtent()
        Else
            RequiresResampling()
        End If
    ...
    Public Function ProcessRaster() As Core.GISDataStructures.Raster
        ...
        If Not String.IsNullOrEmpty(txtRasterPath.Text) Then
            If Core.GISDataStructures.Raster.Exists(txtRasterPath.Text) Then
            ...
            Else
                ...
                If GetSafeOriginalRaster(gRaster) Then
                    ...
                    Dim sWorkspace As String = Core.GISDataStructures.Raster.GetWorkspacePath(txtRasterPath.Text)
                    ...
                    If theDir.Exists Then
                        ...
                        Else
                            eResult = RasterManager.Copy(gRaster.FullPath, txtRasterPath.Text, valCellSize.Value, valLeft.Value, valTop.Value, nRows, nCols, GCD.GCDProject.ProjectManagerUI.GCDNARCError.ErrorString)
                        ...
                        If eResult = RasterManager.RasterManagerOutputCodes.PROCESS_OK Then
                            If GISDataStructures.Raster.Exists(txtRasterPath.Text) Then
                                ...
                                gResult = New GISDataStructures.RasterDirect(txtRasterPath.Text)
                                ...
                                If m_ePurpose = ImportRasterPurposes.DEMSurvey AndAlso valPrecision.Enabled Then
                                    Try
                                        'If the project units have not yet been written to 
                                        If GCD.GCDProject.ProjectManager.CurrentProject.DisplayUnits Is Nothing Then
                                            Dim pLinearUnits As LinearUnitClass = New LinearUnitClass(GISCode.NumberFormatting.GetLinearUnitsFromESRI(gRaster.LinearUnits))
                                            GCD.GCDProject.ProjectManager.CurrentProject.DisplayUnits = pLinearUnits.ToString
                                        End If

                                        'If the coordinate system has not yet been written to 
                                        If GCD.GCDProject.ProjectManager.CurrentProject.CoordinateSystem Is Nothing Then
                                            Dim sCoordinateSystem As String = gRaster.SpatialReference.Name
                                            GCD.GCDProject.ProjectManager.CurrentProject.CoordinateSystem = sCoordinateSystem
                                        End If

                                        GCD.GCDProject.ProjectManager.CurrentProject.Precision = CInt(valPrecision.Value)
                            ...
                            If m_ePurpose = ImportRasterPurposes.DEMSurvey Then
                                ...
                                Dim sHillshadePath As String = GCD.GCDProject.ProjectManager.OutputManager.DEMSurveyHillShadeRasterPath(txtName.Text)
                        ...
                        Else
                            ...
                            Dim exInner As New Exception(GCD.GCDProject.ProjectManagerUI.GCDNARCError.ErrorString.ToString)
                            Dim ex As New Exception("The copy raster routine in Raster Manager DLL failed. ", exInner)
                            ex.Data("Input raster") = gRaster.FullPath
    ...
    Private Sub UpdateOutputExtent()
        ...
        Try
            ...
            If TypeOf m_OriginalExtent Is GISDataStructures.ExtentRectangle Then
                valTop.Minimum = Decimal.MinValue
                valLeft.Minimum = Decimal.MinValue
                valBottom.Minimum = Decimal.MinValue
                valRight.Minimum = Decimal.MinValue
    ...
    Private Sub UpdateOriginalRasterExtentFormatting()
        ...
        If TypeOf m_OriginalExtent Is Core.GISDataStructures.ExtentRectangle AndAlso valCellSize.Value > 0 Then
    ...
    Private Function RequiresResampling() As Boolean
        ...
        If GetSafeOriginalRaster(gOriginalRaster) Then
            ...
            If fNewCellSize = fOldCellSize Then
                If Math.Round(Math.IEEERemainder(gOriginalRaster.Extent.Left, fNewCellSize), CInt(valPrecision.Value) + 1) = 0 Then
                    If Math.Round(Math.IEEERemainder(gOriginalRaster.Extent.Top, fNewCellSize), CInt(valPrecision.Value) + 1) = 0 Then
                        bResult = False
                    End If
                End If
            End If
            'Else
            'Throw New Exception("Unable to determine result because there is no raster selected.")
        End If
    ...
    Private Sub cmdSave_Click(sender As System.Object, e As System.EventArgs) Handles cmdSave.Click
        txtRasterPath.Text = Core.GISDataStructures.Raster.BrowseSave("Output Raster", , GISCode.FileSystem.RemoveDangerousCharacters(txtName.Text))
    End Sub
    ...
    Private Sub cmdHelp_Click(sender As System.Object, e As System.EventArgs) Handles cmdHelp.Click
        ...
        Select Case m_ePurpose
            Case ImportRasterPurposes.StandaloneTool
                Process.Start(My.Resources.HelpBaseURL & "gcd-command-reference/data-prep-menu/a-clean-raster-tool")

            Case ImportRasterPurposes.DEMSurvey
                Process.Start(My.Resources.HelpBaseURL & "gcd-command-reference/data-prep-menu/d-add-dem-survey")

            Case ImportRasterPurposes.AssociatedSurface
                Process.Start(My.Resources.HelpBaseURL & "gcd-command-reference/gcd-project-explorer/d-dem-context-menu/iv-add-associated-surface/1-loading-user-defined-associated-surface")

            Case ImportRasterPurposes.ErrorCalculation
                Process.Start(My.Resources.HelpBaseURL & "gcd-command-reference/gcd-project-explorer/g-error-surfaces-context-menu/i-specify-error-surface")
