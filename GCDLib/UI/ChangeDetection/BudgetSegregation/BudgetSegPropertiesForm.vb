Public Class BudgetSegPropertiesForm

    Private m_pArcMap As ESRI.ArcGIS.Framework.IApplication
    Private m_nBSID As Integer
    Private m_bsOutputs As GCD.BudgetSegregation.BudgetSegregationOutputsClass
    ...
    Public Sub New(pArcMap As ESRI.ArcGIS.Framework.IApplication, rDoD As ProjectDS.DoDsRow)
        ...
        ucPolygon.Initialize(pArcMap, "Budget Segregation Polygon Mask", GISDataStructures.BrowseVectorTypes.Polygon)
    End Sub
    ...
    Private Sub BudgetSegPropertiesForm_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        For Each rDoD As ProjectDS.DoDsRow In GCD.GCDProject.ProjectManager.CurrentProject.GetDoDsRows
            Dim nIndex As Integer = cboDoD.Items.Add(New ListItem(rDoD.DoDID, rDoD.Name))
    ...
    Private Sub cmdOK_Click(sender As System.Object, e As System.EventArgs) Handles cmdOK.Click
        ...
        Try
            ...
            For Each aDoD As ProjectDS.DoDsRow In GCD.GCDProject.ProjectManager.CurrentProject.GetDoDsRows
                If aDoD.DoDID = DirectCast(cboDoD.SelectedItem, ListItem).ID Then
                    rDoD = aDoD
                    'New place to create budget segregation folder
                    Dim sBS_Folder As String = GCD.GCDProject.ProjectManager.OutputManager.CreateBudgetSegFolder(rDoD.Name)
            ...
            If Not TypeOf rDoD Is ProjectDS.DoDsRow Then
                ...
                ex.Data("DoDID") = DirectCast(cboDoD.SelectedItem, ListItem).ID
                Throw ex
            End If

            Dim dodProps As GCD.ChangeDetection.ChangeDetectionProperties = GCD.ChangeDetection.ChangeDetectionProperties.CreateFromDoDRow(rDoD)
            Dim bs As New GCD.BudgetSegregation.BudgetSegregationEngine(dodProps, New IO.DirectoryInfo(txtOutputFolder.Text), My.Settings.ChartHeight, My.Settings.ChartWidth)
            m_bsOutputs = bs.Calculate(ucPolygon.SelectedItem, cboField.Text, My.Settings.ChartWidth, My.Settings.ChartHeight, True)
            Windows.Forms.Cursor.Current = Windows.Forms.Cursors.WaitCursor

            Dim sRelativeFolder As String = GCD.GCDProject.ProjectManager.GetRelativePath(txtOutputFolder.Text)
            Dim sPCDepositionVolPie As String = GCD.GCDProject.ProjectManager.GetRelativePath(m_bsOutputs.PieCharts.PercentageTotalDepositionVolumePiePath)
            Dim sPCErosionVolPie As String = GCD.GCDProject.ProjectManager.GetRelativePath(m_bsOutputs.PieCharts.PercentageTotalErosionVolumePiePath)
            Dim sPCTotalVolPie As String = GCD.GCDProject.ProjectManager.GetRelativePath(m_bsOutputs.PieCharts.PercentageTotalVolumePiePath)
            Dim sClassLegend As String = GCD.GCDProject.ProjectManager.GetRelativePath(m_bsOutputs.ClassLegendPath)
            Dim sClassSummary As String = GCD.GCDProject.ProjectManager.GetRelativePath(m_bsOutputs.ClassSummaryPath)

            Dim bsRow As ProjectDS.BudgetSegregationsRow = GCD.GCDProject.ProjectManager.ds.BudgetSegregations.AddBudgetSegregationsRow(rDoD, txtName.Text, m_bsOutputs.PolygonMask, cboField.Text, sRelativeFolder, _
            ...
            For Each sMaskName As String In m_bsOutputs.MaskOutputs.Keys
                ...
                Dim aMask As GCD.BudgetSegregation.BudgetSegregationOutputsClass.MaskOutputClass = m_bsOutputs.MaskOutputs(sMaskName)

                Dim sMaskCSV As String = GCD.GCDProject.ProjectManager.GetRelativePath(aMask.csvFilename)
                Dim sSummaryFile As String = GCD.GCDProject.ProjectManager.GetRelativePath(aMask.SummaryPath)
                Dim sAreaHistPath As String = GCD.GCDProject.ProjectManager.GetRelativePath(aMask.AreaChartPath)
                Dim sVolHistPath As String = GCD.GCDProject.ProjectManager.GetRelativePath(aMask.VolumeChartPath)

                GCD.GCDProject.ProjectManager.ds.BSMasks.AddBSMasksRow(bsRow, aMask.MaskValue, sMaskName, _
                                                                       aMask.ChangeStats.AreaErosion_Raw, aMask.ChangeStats.AreaDeposition_Raw, _
                                                                       aMask.ChangeStats.AreaErosion_Thresholded, aMask.ChangeStats.AreaDeposition_Thresholded, _
                                                                       aMask.ChangeStats.VolumeErosion_Raw, aMask.ChangeStats.VolumeDeposition_Raw, _
                                                                       aMask.ChangeStats.VolumeErosion_Thresholded, aMask.ChangeStats.VolumeDeposition_Thresholded, _
                                                                       aMask.ChangeStats.VolumeErosion_Error, aMask.ChangeStats.VolumeDeposition_Error, _
    ...
    Private Function ValidateForm() As Boolean
        ...
        If TypeOf ucPolygon.SelectedItem Is GISDataStructures.PolygonDataSource Then
            ...
            If TypeOf cboDoD.SelectedItem Is ListItem Then
                Dim nDoD As Integer = DirectCast(cboDoD.SelectedItem, ListItem).ID
                Dim rDoD As ProjectDS.DoDsRow = Nothing
                Dim bDoDExists As Boolean = False
                For Each rItem As ProjectDS.DoDsRow In GCD.GCDProject.ProjectManager.ds.DoDs
                ...
                If TypeOf rDoD Is ProjectDS.DoDsRow Then
                    Dim sDoDPath As String = GCD.GCDProject.ProjectManager.GetAbsolutePath(rDoD.RawDoDPath)
                    If GISDataStructures.Raster.Exists(sDoDPath) Then
                        Dim gDoD As New GISDataStructures.RasterDirect(sDoDPath)
                        ...
                        If bMissingSpatialReference Then
                            MsgBox("The selected feature class appears to be missing a spatial reference. All GCD inputs must possess a spatial reference and it must be the same spatial reference for all datasets in a GCD project." & _
                              " If the selected feature class exists in the same coordinate system, " & gDoD.SpatialReference.Name.ToString & ", but the coordinate system has not yet been defined for the feature class." & _
                        ...
                        Else
                            If Not gDoD.CheckSpatialReferenceMatches(ucPolygon.SelectedItem.SpatialReference) Then
                                ...
                                MsgBox("The coordinate system of the selected feature class:" & vbCrLf & vbCrLf & ucPolygon.SelectedItem.SpatialReference.Name.ToString & vbCrLf & vbCrLf & "does not match that of the GCD project:" & vbCrLf & vbCrLf & gDoD.SpatialReference.Name.ToString & vbCrLf & vbCrLf & _
                     "All datasets within a GCD project must have the identical coordinate system. However, small discrepencies in coordinate system names might cause the two coordinate systems to appear different. " & _
                     "If you believe that the selected dataset does in fact possess the same coordinate system as the GCD project then use the ArcToolbox 'Define Projection' geoprocessing tool in the " & _
                    ...
                    If rDoD.DoDID = nDoDID Then
                        ...
                        If rDoD.TypeMinLOD Then
                            ...
                            If Not rDoD.IsThresholdNull Then
                                txtUncertaintyAnalysis.Text &= ", Threshold at " & rDoD.Threshold.ToString("#.00") & GCD.GCDProject.ProjectManager.CurrentProject.DisplayUnits
                            End If
    ...
    Private Sub UpdateOutputFolder(ByVal sDoDName As String)
        ...
        txtOutputFolder.Text = GCD.GCDProject.ProjectManager.OutputManager.GetBudgetSegreationDirectoryPath(sDoDName)
                        ...
                        If nDoDID > 0 Then
                            For Each rDoD As ProjectDS.DoDsRow In GCD.GCDProject.ProjectManager.CurrentProject.GetDoDsRows
                                If nDoDID = rDoD.DoDID Then
                                    txtOutputFolder.Text = GCD.GCDProject.ProjectManager.OutputManager.GetBudgetSegreationDirectoryPath(rDoD.Name)
    ...
    Private Sub PolygonChanged(ByVal sender As Object, ByVal e As GISCode.UserInterface.InputUCSelectedItemChangedEventArgs) Handles ucPolygon.SelectedItemChanged
        If TypeOf ucPolygon.SelectedItem Is GISDataStructures.PolygonDataSource Then
