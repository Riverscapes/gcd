...
Public Class DodPropertiesUC
    ...
    Private Sub DodPropertiesUC_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        For Each aControl In Me.Controls
            If TypeOf aControl Is TextBox Then
                DirectCast(aControl, TextBox).BorderStyle = Windows.Forms.BorderStyle.None
            End If
        Next
        ...
        If TypeOf m_rDoD Is ProjectDS.DoDsRow Then
            ...
            If m_rDoD.TypePropagated OrElse m_rDoD.TypeProbabilistic Then
                If Not m_rDoD.IsPropagatedErrorRasterPathNull Then
                    txtPropErr.Text = GCD.GCDProject.ProjectManager.GetAbsolutePath(m_rDoD.PropagatedErrorRasterPath)
                End If
            End If
            ...
            If m_rDoD.TypeProbabilistic Then
                ...
                If Not m_rDoD.IsProbabilityRasterNull Then
                    txtProbabilityRaster.Text = GCD.GCDProject.ProjectManager.GetAbsolutePath(m_rDoD.ProbabilityRaster)
                End If

                If Not m_rDoD.IsSpatialCoErosionRasterNull AndAlso Not String.IsNullOrEmpty(m_rDoD.SpatialCoErosionRaster) Then
                    txtErosionalSpatialCoherenceRaster.Text = GCD.GCDProject.ProjectManager.GetAbsolutePath(m_rDoD.SpatialCoErosionRaster)
                End If

                If Not m_rDoD.IsSpatialCoDepositionRasterNull AndAlso Not String.IsNullOrEmpty(m_rDoD.SpatialCoDepositionRaster) Then
                    txtDepositionSpatialCoherenceRaster.Text = GCD.GCDProject.ProjectManager.GetAbsolutePath(m_rDoD.SpatialCoDepositionRaster)
                End If
                ...
                If Not m_rDoD.IsPosteriorRasterNull AndAlso Not String.IsNullOrEmpty(m_rDoD.PosteriorRaster) Then
                    txtPosteriorRaster.Text = GCD.GCDProject.ProjectManager.GetAbsolutePath(m_rDoD.PosteriorRaster)
                End If
    ...
    Private Sub AddToMapToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles _
        ...
        If cms.SourceControl.Name.ToLower.Contains("dem") Then
            ...
            For Each aDEMSurvey In m_rDoD.ProjectRow.GetDEMSurveyRows
                If String.Compare(sItemName, aDEMSurvey.Name, True) = 0 Then
                    GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddDEM(aDEMSurvey)
                    Exit Sub
                End If
            Next
            ...
            For Each aDEMSurvey In m_rDoD.ProjectRow.GetDEMSurveyRows
                For Each anErrorSurface In aDEMSurvey.GetErrorSurfaceRows
                    If String.Compare(anErrorSurface.Name, sItemName, True) = 0 Then
                        GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddErrSurface(anErrorSurface)
                        Exit Sub
                    End If
                Next
            Next
        Else
            ...
            If Not String.IsNullOrEmpty(sPath) Then
                If GISDataStructures.Raster.Exists(sPath) Then
                    ...
                    Select Case sFileName
                        ...
                        Case "PropErr"
                            GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddPropErr(m_rDoD)

                        Case "priorProb"
                            GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddProbabilityRaster(m_rDoD, m_rDoD.ProbabilityRaster)

                        Case "nbrErosion"
                            GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddProbabilityRaster(m_rDoD, m_rDoD.SpatialCoErosionRaster)

                        Case "nbrDeposition"
                            GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddProbabilityRaster(m_rDoD, m_rDoD.SpatialCoDepositionRaster)

                    End Select



                    'If sFileName.Contains("priorProb") Then
                    '    'TODO: Implement the use of the probability layer and or create an internal probability layer and implement that
                    '    'GCD.GCDProject.ProjectManagerUI.ArcMapManager.AddAssociatedSurface()
                    '    GISCode.ArcMap.AddToMap(My.ThisApplication, sPath, IO.Path.GetFileNameWithoutExtension(sPath), GISDataStructures.BasicGISTypes.Raster)
                    'End If
                End If
            End If
    ...
    Private Sub cmsBasicRaster_Opening(sender As System.Object, e As System.ComponentModel.CancelEventArgs) Handles cmsBasicRaster.Opening
