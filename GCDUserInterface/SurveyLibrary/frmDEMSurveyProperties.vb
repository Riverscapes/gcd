...
#Region "Survey Property Routines"
        ...
        Private Sub SurveyPropertiesForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            InitControls()
            cboSingle.Items.AddRange(ProjectManagerBase.SurveyTypes.Values.ToArray())

            DEMSurveyBindingSource.DataSource = ProjectManagerBase.ds
            DEMSurfaceBindingSource.DataSource = ProjectManagerBase.ds
            ErrorTableBindingSource.DataSource = ProjectManagerBase.ds
            ...
            txtFolder.Text = ProjectManagerBase.OutputManager.DEMSurveyFolder(demRow.Name)
            ...
            Else
                For i As Integer = 0 To cboSingle.Items.Count - 1
                    If String.Compare(DirectCast(cboSingle.Items(i), SurveyType).Name, demRow.SingleMethodType, True) = 0 Then
        ...
        Private Sub InitControls()
            ...
            cmdAddDEMToMap.Visible = ProjectManagerUI.IsArcMap
            cmdAddAssocToMap.Visible = ProjectManagerUI.IsArcMap
            cmdAddErrorToMap.Visible = ProjectManagerUI.IsArcMap
        ...
        Private Function ValidateForm() As Boolean
            ...
            For Each aDEM As ProjectDS.DEMSurveyRow In ProjectManagerBase.ds.DEMSurvey
                ...
                If Not String.IsNullOrEmpty(txtName.Text) Then
                    If TypeOf m_ImportRasterform Is frmImportRaster Then
                        If TypeOf m_ImportRasterform.ucRaster.SelectedItem Is GCDConsoleLib.Raster Then
                            sRasterPath = ProjectManagerBase.OutputManager.DEMSurveyRasterPath(txtName.Text).FullName
                        End If
                    End If
                End If
        ...
        Private Sub btnBrowseFile_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAddDEMToMap.Click
            ...
            '    Dim sHillShade As String = GCD.GCDProject.ProjectManagerBase.OutputManager.DEMSurveyHillShadeRasterPath(txtName.Text)
        ...
        Private Sub btnBrowseMask_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBrowseMask.Click
            ...
            Try
                ...
                If TypeOf gNewMask Is GCDConsoleLib.Vector Then
                    ...
                    Dim gDEM As New GCDConsoleLib.Raster(ProjectManagerBase.GetAbsolutePath(demRow.Source))
                    If Not gDEM.Proj.IsSame(gNewMask.Proj) Then
                        MsgBox("The spatial reference of the selected polygon feature class does not match that of the DEM survey raster (" & gDEM.Proj.PrettyWkt & ").", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong)
                        Exit Sub
                    End If

                    ' Check that the new mask has at least one feature
                    If gNewMask.Features.Count < 1 Then
                        MsgBox("The polygon feature class is empty and contains no features.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong)
                        Exit Sub
                    End If
                    ...
                    If Not bContainsAtLeastOneStringField Then
                        MsgBox("The polygon feature class does not contain at least one field of type string, needed for capturing the survey method.", MsgBoxStyle.Information, GCDCore.Properties.Resources.ApplicationNameLong)
                        Exit Sub
                    End If
        ...
        Private Sub LoadRasterProperties()
            ...
            Dim sAbsolutePath As IO.FileInfo = ProjectManagerBase.GetAbsolutePath(txtRasterPath.Text)
            If Not sAbsolutePath.Exists Then
                Exit Sub
            End If
            ...
            Try
                ' File Size
                If sAbsolutePath.Exists Then
                    sRasterProperties &= vbNewLine & "Raster file size: " & (sAbsolutePath.Length / 1048576).ToString("#,##0.00") & "Mb"
                End If
            Catch ex As Exception
                ' Do nothing. 
            End Try
...
#Region "Associated Surface Events"
        ...
        Public Shared Sub DeleteAssociatedSurface(rAssoc As ProjectDS.AssociatedSurfaceRow)
            ...
            Dim response As MsgBoxResult = MsgBox("Are you sure you want to remove the selected associated surface from the GCD Project? This will also delete the raster associated with this surface.", MsgBoxStyle.YesNo Or MsgBoxStyle.Question, GCDCore.Properties.Resources.ApplicationNameLong)
            If response = MsgBoxResult.Yes Then
                Dim sPath As IO.FileInfo = ProjectManagerBase.GetAbsolutePath(rAssoc.Source)
...
#Region "Error Calculation Events"
        ...
        Private Sub btn_AddErrorSurface_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddError.Click
            ...
            Try
                Dim demRow As ProjectDS.DEMSurveyRow = ProjectManagerBase.ds.DEMSurvey.FindByDEMSurveyID(m_DEMSurveyID)
                DEMSurveyBindingSource.EndEdit()
        ...
        Public Shared Function SpecifyErrorSurface(rDEM As ProjectDS.DEMSurveyRow) As ProjectDS.ErrorSurfaceRow
            ...
            Dim gDEM As New GCDConsoleLib.Raster(ProjectManagerBase.GetAbsolutePath(rDEM.Source))
            ...
            If frm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                ...
                If TypeOf gRaster Is GCDConsoleLib.Raster Then
                    Try
                        Dim sRasterPath As String = ProjectManagerBase.GetRelativePath(frm.txtRasterPath.Text)
                        rError = ProjectManagerBase.ds.ErrorSurface.AddErrorSurfaceRow(frm.txtName.Text, "Imported Raster", sRasterPath, rDEM)
                        ProjectManagerBase.save()
        ...
        Public Shared Sub DeleteErrorSurface(rError As ProjectDS.ErrorSurfaceRow)
            ...
            Dim response As MsgBoxResult = MsgBox("Are you sure you want to delete the selected error surface from the GCD project? This will also delete the raster associated with this error surface.", MsgBoxStyle.YesNo Or MsgBoxStyle.Question, GCDCore.Properties.Resources.ApplicationNameLong)
            If response = MsgBoxResult.Yes Then
                Dim sPath As IO.FileInfo = ProjectManagerBase.GetAbsolutePath(rError.Source.ToString)
                Dim bContinue As Boolean = True
            ...
            If bContinue Then
                Try
                    ProjectManagerBase.GetAbsolutePath(rError.Source).Directory.Delete()
                Catch ex As Exception
                    ' do nothing
                End Try
            End If
